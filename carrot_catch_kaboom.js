...
let currentCarrotSpeed = BASE_CARROT_SPEED; // This is the correct variable name
...
scene("game", () => {
    ...
    onUpdate(() => {
        ...
        const MOVE_AMOUNT = 3; // TEST: Fixed move amount per frame
    ...
    // <<<< CORRECTED TYPO IN SPEED UP LOGIC >>>>
    player.onCollide("carrot",(c)=>{
        play("sfx_player_catch",{volume:0.6});
        destroy(c);
        score+=10;
        scoreText.text=`Score: ${score}`;
        carrotsCollectedForSpeedUp++;
        if(carrotsCollectedForSpeedUp>=CARROTS_PER_SPEED_UP){
            play("sfx_speed_up",{volume:0.7});
            currentCarrotSpeed *= SPEED_INCREASE_FACTOR; // Corrected variable name
            currentGoblinSpeed *= SPEED_INCREASE_FACTOR;
