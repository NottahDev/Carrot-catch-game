...
loadSound("sfx_yay_extra_life", "sfx/yay.mp3"); // <<<< NEW: Load "yay" sound effect
...
scene("game", () => {
    ...
    // Player catches extra life heart
    player.onCollide("extraLife", (lifeUp) => {
        destroy(lifeUp);
        if (lives < MAX_LIVES) {
            lives++;
            play("sfx_yay_extra_life", { volume: 0.8 }); // <<<< PLAY "YAY" SOUND
            updateLifeHeartsUI();
        } else {
            score += 25;
            scoreText.text = `Score: ${score}`;
            play("sfx_player_catch", { volume: 0.5 }); // Regular catch sound for points if at max lives
        }
    });
