...
loadSprite("heart_empty", "img/heart_empty.png"); // <<<< NEW: Load empty heart sprite
loadSprite("score_icon", "img/score.png");      // <<<< NEW: Load score icon sprite
...
let lifeHeartsUI = [];
let scoreTextUI = null; // <<<< NEW: To hold reference to score text UI element
let scoreIconUI = null; // <<<< NEW: To hold reference to score icon UI element
...
scene("game", () => {
    ...
    let touchingLeft = false; let touchingRight = false;
    ...
    function updateScoreDisplay() { // <<<< NEW function to update score UI
        if (scoreTextUI) {
            scoreTextUI.text = `${score}`; // Only update the text part
        }
    }
    ...
    function updateLifeHeartsUI(){
        lifeHeartsUI.forEach(h=>destroy(h));
        lifeHeartsUI=[];
        for(let i=0;i<MAX_LIVES;i++){
            const heartSpriteName = (i < lives) ? "heart" : "heart_empty"; // <<<< Use heart_empty
            lifeHeartsUI.push(add([
                sprite(heartSpriteName),
                pos(width()-10-(i*30),10), // Position from right
                anchor("topright"),
                scale(0.7),fixed(),z(10)
            ]));
        }
    }
    updateLifeHeartsUI();
    ...
    onCollide("carrot","ground",(c,g)=>{destroy(c);loseLife();});

    // <<<< SCORE UI WITH ICON >>>>
    scoreIconUI = add([
        sprite("score_icon"),
        pos(10, 10), // Position icon
        scale(0.8), // Adjust scale as needed
        fixed(),
        z(10)
    ]);
    scoreTextUI = add([
        text(`${score}`, { size: 24, font: "arial" }), // Just the number
        pos(10 + scoreIconUI.width * 0.8 + 5, 10 + (scoreIconUI.height*0.4)), // Position text next to icon, vertically centered
        anchor("left"), // Anchor text from its left middle
        fixed(),
        z(10)
    ]);
    // Removed the old livesText here as it's replaced by hearts
});
